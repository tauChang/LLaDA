...
class HFLM(TemplateLM):
    ...
    def _model_generate(self, context, max_length, stop, **generation_kwargs):
        ...
        # start_peak_mem = torch.cuda.max_memory_allocated()
        ...
        end_time = time.time()
        end_peak_mem = torch.cuda.max_memory_allocated()
        peak_mem = end_peak_mem / 1024 / 1024 / 1024

        num_generated_tokens = outputs.shape[1] - context.shape[1]
        batch_size = context.shape[0]
        total_generated_tokens = num_generated_tokens * batch_size
        time_taken = end_time - start_time
        throughput = total_generated_tokens / time_taken if time_taken > 0 else float("inf")

        stats["throughput"].append(throughput)
        # stats["used_mem"].append(used_mem)
        stats["peak_mem"].append(peak_mem)
...
if __name__ == "__main__":
    ...
    # print(f"average used memory: {np.mean(stats['used_mem'])}")
